...
void log_received_all_started ( proc_t* proc ) {
    log_output( fd_event, log_received_all_started_fmt, get_physical_time(), proc->id );
}

void log_received_all_done ( proc_t* proc ) {
    log_output( fd_event, log_received_all_done_fmt, get_physical_time(), proc->id );
}
...
void log_transfer_out ( TransferOrder* trans ) {
    log_output( fd_event, log_transfer_out_fmt, get_physical_time(),
            trans->s_src, trans->s_amount, trans->s_dst );
}

void log_transfer_in ( TransferOrder* trans ) {
    log_output( fd_event, log_transfer_in_fmt, get_physical_time(),
